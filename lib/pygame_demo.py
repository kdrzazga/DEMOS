    ...
    * :meth:`step`  - advance and render a single frame; only called while the
    ...
    def __init__(self, width, height, title, fps=60, opengl=True,
        ...
        if self.windowed:
            ...
            # Fullscreen at the monitor's own resolution: passing (0, 0) makes SDL
            # use the current desktop mode, so the GL drawable, the reported size and
            # the viewport all agree. Requesting a fixed non-native size instead let
            # the packaged exe's SDL render into a smaller drawable than the source
            # did, which shifted everything sideways. The content is authored at
            # width:height and simply fills the screen -- no letterboxing, no
            # adapting to other aspect ratios.
            flags |= FULLSCREEN
            pygame.display.set_mode((0, 0), flags)
        pygame.display.set_caption(self.title)
        if opengl:
            glViewport(0, 0, *pygame.display.get_surface().get_size())
